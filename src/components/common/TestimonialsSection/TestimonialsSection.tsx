...
const TestimonialsSection: React.FC = () => {
    // 1. Наш стейт для текущего слайда (замена let currentSlide = 0)
    const [currentSlide, setCurrentSlide] = useState<number>(0);

    // Массив с данными слайдов, чтобы не дублировать верстку 5 раз (улучшение архитектуры)
    const slidesData = [
        {
            name: "Eleanor Pena",
    ...
    // 2. Функция "Вперед" (замена вашей nextSlide)
    const nextSlide = () => {
        setCurrentSlide((prev) => (prev + 1) % totalSlides);
    };

    // 3. Функция "Назад" (замена вашей prevSlide)
    const prevSlide = () => {
                ...
                <div className={styles.testimonialsContainer}>
                    ...
                    <div className={styles.testimonialsContent}>
                        ...
                        <div className={styles.sliderContainer}>
                            {/* 4. Рендерим ТОЛЬКО один активный слайд (замена item.style.display) */}
                    ...
                    {/* 5. Индикаторы пагинации (замена indicators.forEach) */}
